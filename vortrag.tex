...
\begin{frame}[t]{Beispiel: Reeb-Graph von $\mathcal{M}$}
    \begin{center}
        \uncover<1>{\includegraphics[width=1.05\textwidth]{Bilder/manifold.pdf}}
        \mbox{ }\hfill {\small Quelle: \cite[Fig.~2]{buchin2015}}\\[1em]
        $\mathcal{M}$ ist eine 3-dimensionale Mannigfaltigkeit mit Rand \textcolor{gray}{in den meisten Fällen}
    \end{center}
\end{frame}
...
\begin{frame}{Knoten des Reeb-Graphen}
    \begin{columns}
        \column{.6\textwidth}
        Wir nehmen im weiteren Verlauf stets folgendes an:
        \begin{itemize}
        ...
        \end{itemize}
        Damit können wir die Knoten des Reeb-Graphen in vier Typen unterteilen.
        \column{.4\textwidth}
        \centering
        \begin{tabular}{lcc}
            \toprule
            ...
            \bottomrule
        \end{tabular}\\[2em]
        \includegraphics[width=\textwidth]{Bilder/manifold_right.pdf}
    \end{columns}
\end{frame}
