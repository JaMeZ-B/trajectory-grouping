...
\AtBeginSection[]{%
  \begin{frame}[c]
  \begin{beamercolorbox}[sep=12pt,shadow=true,center]{block title}
...
\newcommand{\includefilm}[2]{%
    \movie[autostart]{\begin{tikzpicture}%
        \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=#2]{videos/#1.png}};
        \begin{scope}[x={(image.south east)},y={(image.north west)}]%
            \node[white] at (.5,.5) {\scalebox{2}{\Huge\faPlayCircle}};
        \end{scope}%
    \end{tikzpicture}}{videos/#1.mov}%
}
...
\begin{frame}{Einleitung/Warum ist das interessant?}
    ...
    \begin{itemize}[<+->]
        ...
        \item Analyse von Verkehrsteilnehmern
    \end{itemize}
    ...
    \begin{enumerate}
        ...
        \item<7-> Finde einen Weg, die so definierten \enquote{Gruppen} effizient zu berechnen
    \end{enumerate}
\end{frame}
...
\begin{frame}{Ziel dieses Vortrags}
    \vspace{-.6em}
    \begin{center}%
        \includefilm{trajectories_400_818_3_100_4}{1.19\textheight}\,
        \rotatebox{90}{\tiny\copyright Frank \textsc{Staals}, \url{https://fstaals.net/grouping/}}
    \end{center}%
\end{frame}
...
\begin{frame}{Beispiel: Skatenight in Münster}
    \begin{minipage}{\textwidth}
        ...
        \rotatebox{90}{\tiny Quelle: \url{allesmuenster.de}}
    \end{minipage}
    \begin{itemize}
...
\begin{frame}[t]{Räumliche Nähe}
    ...
    \begin{definition}<2->
        Zwei Entitäten $x$ und $y$ sind zum Zeitpunkt $t$ \bet{direkt zusammenhängend}, falls sich ihre $\varepsilon$-Bälle zum Zeitpunkt $t$ überlappen.
...
\begin{frame}{Beispiel: Maximale Gruppen}
    \begin{center}
...
\begin{frame}[t]{Weiterer Verlauf}
    \pause
    \begin{itemize}[<+->]
...
\begin{frame}{Identifikation der Knoten im Reeb-Graph}
    ...
    \end{itemize}
    \onslide<7->{Weitere Details: Siehe Ausarbeitung, \textcite{compTopo}, \textcite{MilnorMorse} \pause\vspace{1em}}
    \begin{center}
...
\begin{frame}{Sweep zur Berechnung von $\mathcal{R}$}
    \begin{itemize}[<+->]
        ...
        \item Knoten im Reeb-Graph entsprechen kritischen Werten, also braucht man eigentlich nur ein Liste ebendieser
        \item Da sich die kritischen Werte nicht so einfach berechen lassen, nimmt man folgende Obermenge:
    \end{itemize}
    \begin{block}<6->{Queue}
    ...
    \end{block}
    \begin{itemize}
        \item<7-> Diese Liste mit $\mathcal{O}(\tau n^2)$ Ereignissen lässt sich in $\mathcal{O}(\tau n^2 \log n)$ Zeit berechnen.
    \end{itemize}
\end{frame}
...
\begin{frame}%{Die $xy$-Struktur}
    \begin{block}{$xy$-Struktur}\pause
        \begin{itemize}[<+->]
            ...
            \item Gewicht einer Kante ist der Zeitpunkt, and dem die Kante wieder entfernt wird.
            \item Maximaler Spannbaum $F$ zur Verwaltung der Zusammenhangskomponenten von $G$.
        \end{itemize}
    \end{block}
    \begin{itemize}
        \item<5-> Geschicktes Ausnutzen der Gewichte und geeignete Datenstruktur für $F$ ermöglicht Updates von $G$ \emph{und} $F$ in $\mathcal{O}(\log n)$ Zeit, siehe Ausarbeitung bzw. \cites{buchin2015}{parsaReeb}.
...
\begin{frame}{Berechnung der maximalen Gruppen}
    \begin{itemize}[<+->]
        ...
        \item Lösung: Speichere die maximalen Gruppen zu jeder Kante in $\mathcal{R}$ in einen Baum, um $\mathcal{R}$ nicht zusätzlich \enquote{rückwärts} traversieren zu müssen.
        \item Details: Siehe Ausarbeitung und \textcite{buchin2015}
    \end{itemize}
    \begin{satz}<6->[{\cite[Thm.~9]{buchin2015}}]
        Alle maximalen Gruppen lassen sich in $\mathcal{O}(\tau n^3 + N)$ Zeit berechnen, wobei $N \in \mathcal{O}(\tau n^4)$ die Ausgabegröße ist.
    \end{satz}
\end{frame}
...
\begin{frame}{Evaluation}
    \begin{columns}[t]
        \column{.4\textwidth}
        \begin{itemize}
            \item Positionsdaten von Hirschen, Elchen und Rindern im Starkey Wald in Oregon.
            \item Zeitraum: 30 Tage
            \item 126 Trajektorien mit je 1264 Positionen
            \item Trackingdaten unvollständig $\to$ Sampling der Daten war nötig
            \item $\varepsilon=\SI{200}{\metre}$, $\delta = \SI{4}{\hour}$, $m=\num{5}$
        \end{itemize}
        Man stellt fest, dass die Gruppierungen menschlicher Intuition entsprechen.\\[1.4em]
        {\footnotesize\copyright Frank \textsc{Staals},\\[-1ex]
        \url{https://fstaals.net/grouping/}}
        \column[T]{.56\textwidth}
        \vspace{-3em}
        \rule{1.5cm}{0pt}\includefilm{starkey}{.86\textwidth}%
    \end{columns}
\end{frame}

\begin{frame}{Evaluation: Variation der Parameter}
    \vspace{-1em}
    \begin{center}
        \rule{-1em}{0pt}\includefilm{side_by_side}{1.05\textwidth}
        {\footnotesize\copyright Frank \textsc{Staals}, \url{https://fstaals.net/grouping/}\hfill}
    \end{center}
\end{frame}
...
\begin{frame}{Ausblick}
    \begin{columns}[t]
        \column{.6\textwidth}
        \begin{itemize}
            \item<1-> Das Modell lässt sich auch in höheren Dimensionen verwenden.
            \item<2-> Modell lässt sich \enquote{robust} gegenüber Störungen machen, siehe \textcite[Sec.~4]{buchin2015}.
            \begin{itemize}
                \item<3-> Einführung eines neuen Parameters $\alpha$, der angibt wie lange eine Störung maximal bestehen darf
                \item<4-> \bet{$\alpha$-approximativ direkt zusammenhängend zur Zeit $t$}: direkt zusammenhängend zu \emph{einer} Zeit $t' \in \benbrace*{t- \alpha/2, t + \alpha/2}$.
                \item<5-> Berechung: Schrittweise Vereinfachung von $\mathcal{R}$ mit Hilfe einer Priority Queue.
                \item<6-> Gesamtkosten erhöhen sich auf $\mathcal{O}(\tau n^3 \log n + N)$
            \end{itemize}
        \end{itemize}
        \column[T]{.3\textwidth}
        \centering
        \vspace{1em}
        \onslide<4->{\includegraphics[width=5.5cm]{Bilder/alpha-component}\\
        {\footnotesize Quelle: \cite[Fig.~11]{buchin2015}}}
    \end{columns}
    \begin{itemize}
        \item<7-> Das Modell wurde inzwischen weiter überarbeitet, siehe \textcite{grouping_improved}.
        \begin{itemize}
            \item<8-> Bei einer sehr dichten Ansammlung von Entitäten entspricht dieses Modell besser der menschlichen Intuition
            \item<9-> Algorithmen mit $\mathcal{O}(\tau^2 n^5 \log n)$ \emph{oder} $\mathcal{O}(\tau n^4 2^n)$ Laufzeit.
        \end{itemize}
    \end{itemize}
\end{frame}
