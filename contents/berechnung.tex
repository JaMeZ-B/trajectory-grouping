...
Dies ist zum Beispiel mit dem in \textcite[Sec.~22.4]{cormenIntroduction}\todo{tatsächlich nachschlagen} erläuterten Algorithmus möglich.
...
Die \enquote{Bottom-Up}-Konstruktion der Bäume $\mathcal{T}_{e_1}$ und $\mathcal{T}_{e_2}$ während der Traversierung kostet dabei pro Knoten in $\mathcal{T}_e$ nur konstante Zeit, sodass der Algorithmus insgesamt Kosten von $\mathcal{O}(n \abs*{\mathcal{R}}) = \mathcal{O}(\tau n^3)$ verursacht.
